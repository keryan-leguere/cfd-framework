...
Y_LABEL = r"$L_r/d$ (-)"       # Y-axis label.
PLOT_TITLE = "Adimensional Recirculation Length"  # Title of the plot (set to None if no title is needed).

# Legend settings
SHOW_LEGEND = True             # Set to False to hide legend.
LEGEND_LOC = 'upper left'     # Location of the legend ('best' places it automatically).
...
OUTPUT_FILENAME = './POST_PROCESSING/FIGURES/Lrb.png'  # Output filename for the saved plot.
...
reference_data = np.loadtxt("./REFERENCES/comparison_Lrd_Re10_40.dat",delimiter=",")
#"Carre,,triangle up,,triangle down,,cross,,plus,,triangle incline,,losange,"

Re = np.array([10,20,30,40])
...
#1. Carre:
ax.plot(Re, reference_data[:,1], linestyle='', marker='s', markerfacecolor='white',markeredgecolor='black',color='black', label=u"Gerouache M-S.",
        linewidth=LINE_WIDTH, markersize=MARKER_SIZE)

#2. Triangle:
ax.plot(Re, reference_data[:,3], linestyle='', marker='^', markerfacecolor='white',markeredgecolor='black',color='black', label=u"Braza M.",
        linewidth=LINE_WIDTH, markersize=MARKER_SIZE)

#3. Losange:
ax.plot(Re, reference_data[:,13], linestyle='', marker='D', markerfacecolor='white',markeredgecolor='black',color='black', label=u"Socolescu L.",
        linewidth=LINE_WIDTH, markersize=MARKER_SIZE)

#4. Triangle vers le bas:
ax.plot(Re, reference_data[:,5], linestyle='', marker='v', markerfacecolor='white',markeredgecolor='black',color='black', label=u"Tuann S-Y.",
        linewidth=LINE_WIDTH, markersize=MARKER_SIZE)

#5. Croix:
ax.plot(Re, reference_data[:,7], linestyle='', marker='x', markerfacecolor='white',markeredgecolor='black',color='black', label=u"Dennis S-C-R.",
        linewidth=LINE_WIDTH, markersize=MARKER_SIZE)

#6. plus:
ax.plot(Re, reference_data[:,9], linestyle='', marker='+', markerfacecolor='white',markeredgecolor='black',color='black', label=u"Henderson R-D.",
        linewidth=LINE_WIDTH, markersize=MARKER_SIZE)

#7. Triangle gauche:
ax.plot(Re, reference_data[:,11], linestyle='', marker='<', markerfacecolor='white',markeredgecolor='black',color='black', label=u"Lange C-F.",
        linewidth=LINE_WIDTH, markersize=MARKER_SIZE)



(Re,Lrb) = np.loadtxt("./POST_PROCESSING/DATA/results.dat",usecols=(0,2),unpack=True)
# Plotting the first line with a solid line and circle markers
ax.plot(Re, Lrb, linestyle='-', marker='o', markerfacecolor='black',markeredgecolor='black',color='black', label=r"Simulation Fine Mesh",
        linewidth=LINE_WIDTH, markersize=MARKER_SIZE)

(Re,Lrb) = np.loadtxt("./POST_PROCESSING/DATA/results.coarse.dat",usecols=(0,2),unpack=True)
# Plotting the first line with a solid line and circle markers
ax.plot(Re, Lrb, linestyle='--', marker='o', markerfacecolor='black',markeredgecolor='black',color='black', label=r"Simulation Coarse Mesh",
        linewidth=LINE_WIDTH, markersize=MARKER_SIZE)
