...
Y_LABEL = r"$C_d$ (-)"       # Y-axis label.
PLOT_TITLE = r"Drag Coefficient $C_d$"  # Title of the plot (set to None if no title is needed).
...
OUTPUT_FILENAME = './POST_PROCESSING/FIGURES/Cd.png'  # Output filename for the saved plot.
...
reference_data = np.loadtxt("./REFERENCES/comparison_Cd_Re10_40.dat",delimiter=",")
...
#3. Losange:
ax.plot(reference_data[:,4], reference_data[:,5], linestyle='', marker='D', markerfacecolor='white',markeredgecolor='black',color='black', label=u"Socolescu L.",
        linewidth=LINE_WIDTH, markersize=MARKER_SIZE)

#4. Triangle vers le bas:
ax.plot(reference_data[:,6], reference_data[:,7], linestyle='', marker='v', markerfacecolor='white',markeredgecolor='black',color='black', label=u"Tuann S-Y.",
        linewidth=LINE_WIDTH, markersize=MARKER_SIZE)

#5. Croix:
ax.plot(reference_data[:,8], reference_data[:,9], linestyle='', marker='x', markerfacecolor='white',markeredgecolor='black',color='black', label=u"Dennis S-C-R.",
        linewidth=LINE_WIDTH, markersize=MARKER_SIZE)

#6. plus:
ax.plot(reference_data[:,10], reference_data[:,11], linestyle='', marker='+', markerfacecolor='white',markeredgecolor='black',color='black', label=u"Henderson R-D.",
        linewidth=LINE_WIDTH, markersize=MARKER_SIZE)

#7. Triangle gauche:
ax.plot(reference_data[:,12], reference_data[:,13], linestyle='', marker='<', markerfacecolor='white',markeredgecolor='black',color='black', label=u"Lange C-F.",
        linewidth=LINE_WIDTH, markersize=MARKER_SIZE)



(Re,Lrb) = np.loadtxt("./POST_PROCESSING/DATA/results.dat",usecols=(0,1),unpack=True)
...
(Re,Lrb) = np.loadtxt("./POST_PROCESSING/DATA/results.coarse.dat",usecols=(0,1),unpack=True)
# Plotting the first line with a solid line and circle markers
