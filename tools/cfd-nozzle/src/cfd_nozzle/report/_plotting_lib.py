...
cfd-nozzle renders its figures with ``cfd_plot`` when that package is
installed, and falls back to plain Matplotlib when it is not: cfd-nozzle must
stay usable when deployed on its own, without the rest of the framework.

``cfd-plot`` is a sibling package in this repository, not a PyPI release, so it
cannot be declared as a normal dependency. Install it alongside cfd-nozzle
with::
