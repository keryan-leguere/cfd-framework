...
rapport (le régime, les titres, les en-têtes de tableau) qui disparaît. La
hiérarchie visuelle repose donc ici sur la **couleur** et sur l'atténuation,
qui se comportent de la même façon partout.

``CFD_NOZZLE_GRAS=1`` rétablit le gras par-dessus les couleurs, pour ceux dont
le terminal le rend correctement.

Les valeurs sont de simples chaînes de style Rich, pas des noms de thème : un
panneau construit par ``cfd_nozzle.report.console`` reste ainsi affichable sur
...
ENV_GRAS = "CFD_NOZZLE_GRAS"

_VRAI = {"1", "true", "vrai", "oui", "yes", "on"}


def gras_actif() -> bool:
    """Vrai si l'utilisateur a demandé le gras via ``CFD_NOZZLE_GRAS``."""
...
#: La réponse elle-même : le régime de fonctionnement, la poussée.
ACCENT = _emphase("green")

#: Fonctionnement nominal, adaptation, contrainte respectée.
OK = _emphase("green")

#: Réserve, régime dégradé, donnée douteuse.
ATTENTION = _emphase("yellow")

#: Échec, tuyère non amorcée, configuration irréalisable.
ERREUR = _emphase("red")

#: En-têtes de colonnes.
ENTETE = _emphase("cyan")

#: En-têtes de colonnes des tableaux secondaires (géométrie, MOC).
...
#: Couleur associée à chaque régime de fonctionnement.
STYLE_REGIME: dict[str, str] = {
    "venturi": ERREUR,
    "choc_interne": ERREUR,
    "sur_detendue": ATTENTION,
    "adaptee": OK,
    "sous_detendue": ATTENTION,
}
